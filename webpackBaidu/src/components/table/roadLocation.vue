	...
	<div id="view">
		<div class="header">
			<div class="header-title">
				...
				<p v-cloak @click="showAll" style="margin: 5px;" :class="[CurrentAll?'selectBg':'defaultBg']">显示所有路段
				</p>
		...
		methods: {
			...
			itemOnclick(position) {
				...
				for (var i = 0; i < this.excelData.length; i++) {
					if (this.excelData[i].系统中路段名称 == roadSectionName) {
						roadSectionCurrent.push(this.excelData[i]);
					}
					if (this.excelData[i].系统中道路名称 == roadSectionName) {
						roadSectionCurrent.push(this.excelData[i]);
					}
				}
			...
			addAddress(excelData) {
				...
				for (let i = 0; i < excelData.length; i++) {
					let point = new BMapGL.Point(excelData[i].初始经度, excelData[i].初始纬度);
					pointArr.push(point);
					let marker = new BMapGL.Marker(point); // 创建标注

					this.map.addOverlay(marker); // 将标注添加到地图中
					marker.setTitle(excelData[i].泊位前缀名 + excelData[i].系统中路段名称 + "初始坐标")
					marker.addEventListener("click", function() {
						let opts = {
							title: this.getTitle()
						};
					...
					let point1 = new BMapGL.Point(excelData[i].结束经度, excelData[i].结束纬度);
					pointArr.push(point1);
					let marker1 = new BMapGL.Marker(point1); // 创建标注

					this.map.addOverlay(marker1); // 将标注添加到地图中
					marker1.setTitle(excelData[i].泊位前缀名 + excelData[i].系统中路段名称 + "结束坐标")
					marker1.addEventListener("click", function() {
						...
						let infoWindow = new BMapGL.InfoWindow(info, opts);
						infoWindow.enableAutoPan();
						that.map.openInfoWindow(infoWindow, point1);
					});
			...
			getRoadSections(excelData) {
				...
				for (var i = 0; i < excelData.length; i++) {
					// arr.push(excelData[i].系统中路段名称)
					// if(!excelData[i].系统中路段名称){
					// 	arr.push(excelData[i].系统中道路名称)
					// }else if(!excelData[i].系统中路段名称){
					// 	arr.push(excelData[i].系统中路段名称)
					// }
					if (excelData[i].系统中路段名称) {
						arr.push(excelData[i].系统中路段名称)
					} else if (excelData[i].系统中道路名称) {
						arr.push(excelData[i].系统中道路名称)
					}
				...
				for (let i = 0; i < hash.length; i++) {
					...
					for (let j = 0; j < excelData.length; j++) {
						...
						// 	arr.push(excelData[i].系统中路段名称)
						// }
						// if (hash[i] == excelData[j].系统中路段名称||hash[i] == excelData[j].系统中路段名称) {
						// 	currentCount++
						// }
