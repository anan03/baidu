	...
	<div id="view">
		<div class="header">
			<div class="header-title">
				...
				<p v-cloak @click="daochu" style="margin: 5px;" :class="[CurrentAll?'selectBg':'defaultBg']">导出</p>
...
<script>
	export default {
		...
		methods: {
			...
			itemOnclick(position) {
				...
				this.addAddress(roadSectionCurrent,position);
				//改变选中状态
			...
			daochu() {
				...
				for (var i = 0; i < this.excelData.length; i++) {
					aoa.push([this.excelData[i].外部编号, this.excelData[i].坐标经度, this.excelData[i].坐标纬度]);
				}
	...
	addAddress(excelData,position) {
				console.log('添加');
				//1.排序
				function sortId(a, b) {
					return a.内部编号 - b.内部编号;
				}
				excelData.sort(sortId);
				...
				for (let i = 0; i < excelData.length; i++) {
					...
					marker.addEventListener('dragend', function (e) {//拖动标注结束
						...
						excelData[i].坐标经度 =this.getPosition().lng;
						excelData[i].坐标纬度 =this.getPosition().lat;

						//3.移动完重新加载(为了连线)
						that.itemOnclick(position);
		...
		//2.连线
		var polyline = new BMapGL.Polyline(pointArr, {strokeColor:"blue", strokeWeight:2, strokeOpacity:0.5});
